...
function onDiceClicked(id)
{
    ...
    if (dice && dice.dice && dice.dice.rollOnClick)
    {
        if (dice.dice.clickHandler)
        {
            dice.dice.clickHandler(id);
        }
        dice.dice.roll();
    }
}
...
class Dice {
    ...
    */
    constructor(divId, rollOnClick, clickHandler)
    {
        this.val = 1;
        this.id = divId;
    ...
    roll(cb)
    {
        ...
        for (let i = 1; i <= 6; ++i)
        {
            self.div.classList.remove('show-' + i);
        }
        setTimeout(() => {
            self.div.classList.add('show-' + self.val);
    ...
    //@return the dice, or false if non-existent
    static getDice(id)
    {
        let dice = id;
        if ('string' === typeof id)
